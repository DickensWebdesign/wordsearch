...
function output() {
    ...
    global $tbl_highscore_button;
    global $check;
    global $tbl_highscore;
    global $rows;
    global $columns;
    global $usedFields;
    global $wordNum;
    ...
    global $db;
    global $db;
	$tbl_highscore_button= array();
	$tbl_highscore = array();
	$form_button_value = array();
	$check = array();
	$tbl_fields = array();
	$game= array();


    $content = "<div class='rel'><div id='game-wrapper'><div class='game-floater'>";
    if ($_SESSION['mobile'] == false) {
        $content .= "<div class='center'><img src = '../img/icons/close-glow.png' class='closer' id = 'game-close' alt = '". $tbl_highscore_button[4] ."' title = '".$check[0]."' />
            <img src = '../img/icons/reload.png' class='closer' id = 'game-restart' alt = '".$form_button_value[12]."' title = '".$check[1]."' />
            </div > ";
    }
    $content .= "<div class='game-header'><div>";
    if ($_SESSION['mobile'] == false) {
        $content .= "<p><span class='user-name'> ". $tbl_fields[1] . $_SESSION['user_name'] . "</span><span class='user-level'> ".$tbl_fields[3] . $_SESSION['level'] . "</span></p>";
    } else {
        $content .= "<p><span class='game-menu content-menu2'>".$form_button_value[11]."</span></p><span class='floater-left'>".$tbl_fields[4]."</span><span class='showTime-inner'></span>";
        $content .= "<span class='ingame-details'><span id='errorbox'>".$tbl_fields[5].$check[2]."</span></div>";
    ...
    if ($_SESSION['mobile'] == false) {
        ...
        $content .= "<img src='../img/icons/stop-watch-icon.png' title='".$tbl_highscore[7]."' alt='".$tbl_highscore[7]."'/>";
        $content .= "<div id='errorbox'>".$tbl_fields[5].$check[2]."</div><hr/><div class='highscore-ingame-box'><p>".$check[3]."</p>";
...
*/
    ...
    if ($_SESSION['mobile'] == false) {
        asort($wordsInMenu);
        $content .= "<div id='game-right'><H2 class='menuHeadline wordheader'>".$game[55]."</H2><hr /><div class='wordbox'>";
        foreach ($wordsInMenu as $key => $val) {
            if ($val != "") {
                $content .= "<p class='wordlist' id='" . $key . "'>" . $val . "</p>";
            }
...
function insert_into_db($content) {
    ...
    if($content) {
        ...
        if(!$stmt->execute()) {
            return '<p class="color-red">'.$error_return[19].'</p>';
        }
        $stmt->close();
    }
    ...
    if($stmt->execute()) {
        ...
        if ($result = $stmt->fetch()) {
            ...
            $stmt->close();
            echo '<p class="color-red">'.$error_return[19].'</p>';
        }
    }
}
...
function start_check() {
    ...
    if($_SESSION['logged_in'] == true) {
        $table = "wordsearch-user-made-games";
        $levels = [
            "Beginner" => "beginner",
            "Easy" => "easy",
            "Middle" => "middle",
        ...
        if ($_SESSION['mobile'] == true) {
            $table = "wordsearch-user-made-games-mobile";
            $levels = [
                "Beginner" => "easy",
                "Easy" => "middle",
                "Middle" => "hard"
